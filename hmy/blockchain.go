...
func (hmy *Harmony) GetBlockSigners(
...
) (shard.SlotList, *bls_interface.Mask, error) {
	...
	pubKeys := make([]bls_interface.PublicKeyWrapper, len(committee.Slots))
	for i, validator := range committee.Slots {
		key, err := bls_interface.BytesToBLSPublicKey(validator.BLSPublicKey[:])
		if err != nil {
			return nil, nil, err
		}
		pubKeys[i] = bls_interface.PublicKeyWrapper{
